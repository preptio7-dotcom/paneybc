...
const isImageFile = (file: File) => {
  ...
  return ['.png', '.jpg', '.jpeg', '.webp', '.gif', '.avif', '.svg'].some((ext) => lower.endsWith(ext))
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeName = sanitizeFilename(file.name || 'question-image')
    const key = `questions/images/${Date.now()}-${safeName}`
