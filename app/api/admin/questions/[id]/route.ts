  ...
  try {
    ...
    if (!question || !options || !explanation) {
      return NextResponse.json(
        { error: 'Question, options, and explanation are required' },
        { status: 400 }
      )
    }
    ...
    if (allowMultiple) {
      ...
      if (invalid) {
        return NextResponse.json(
          { error: 'Correct answers must be between 0 and 3' },
          { status: 400 }
        )
      }
    } else {
    ...
    // ── Cache invalidation ────────────────────────────────────────────────────
    // Question content changed + reports cleared — invalidate everything
    // that could be serving stale question data or stale report IDs
    invalidateCache('questions:pool:')       // chapter question pools
    invalidateCache('questions:page:')       // paginated question results
    invalidateCache('questions:reported:')   // reported IDs cache (reports cleared above)
    invalidateCache('subjects:chapter-counts:') // chapter question counts
    invalidateCache('mock:config:')          // mock test canStart counts
    ...
    if (reports.length > 0) {
      ...
      const uniqueEmails = Array.from(
        new Set(reports.map((report) => report.email).filter(Boolean))
      )
...
export async function DELETE(
  ...
  try {
    ...
    // ── Cache invalidation ────────────────────────────────────────────────────
    // Question deleted — clear all caches that include question data or counts
    invalidateCache('questions:pool:')           // chapter question pools
    invalidateCache('questions:page:')           // paginated question results
    invalidateCache('subjects:chapter-counts:')  // chapter counts (one less now)
    invalidateCache('mock:config:')              // mock canStart counts changed
