...
export default function LoginPage() {
  ...
  // Read returnTo from URL (e.g. /auth/login?returnTo=/community).
  // Strip any absolute URLs to prevent open-redirect attacks.
  const getReturnTo = (role?: string) => {
    if (typeof window === 'undefined') return role === 'admin' ? '/admin' : '/dashboard'
    const params = new URLSearchParams(window.location.search)
    const raw = params.get('returnTo') || ''
    const safe = raw && !raw.startsWith('http') ? raw : null
    return safe ?? (role === 'admin' ? '/admin' : '/dashboard')
  }
  ...
  useEffect(() => {
    ...
    window.location.replace(getReturnTo(user.role))
  }, [loading, user])
